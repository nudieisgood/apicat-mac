<template>
  <div id="sidebar-tree-wrapper">
    <div class="sidebar-tree__operation">
      <action-menu @create="addCollection" @search="keywordSearch" :current-tab-name="'Collection'" />
    </div>
    <!-- 2023/12/15 測試把 tree 高度 props 拿掉，看是否還會有部分資料消失問題 :height="virtualHeight"-->
    <div class="sidebar-tree" v-if="items && items.length > 0">
      <a-tree v-if="tempTreeData && tempTreeData.length > 0" :tree-data="tempTreeData" :field-Names="replaceFields"
        :show-icon="true" @expand="onExpand" :draggable="isDraggable" @drop="onDrop" :expandedKeys="expandedKeys"
        :auto-expand-parent="autoExpandParent" :motion="null">
        <template #title="{ data, tabType, posKey, request, name }">
          <div class="tree-view-item" :class="{ 'selected-node': posKey === selectedKey }">
            <div class="tree-view-left">
              <div class="node-box" @click="openTab(data)">
                <span class="node-box__icon" v-if="tabType === TypeEnum.folder.name"><folder-outlined /></span>
                <span class="node-box__icon" v-else-if="tabType === TypeEnum.example.name">
                  <img :src="require('@/assets/img/icon-example.png')" alt="" />
                </span>
                <span class="node-box__request-method method-label" v-if="request"
                  :class="tabsData.classifyMethodColor(request.method)">{{ request.method }}</span>
                <div class="node-box__name-wrapper">
                  <a-input v-if="isShowRenameInput && posKey === selectedKey" v-model:value="selectedData.name"
                    @blur="onBlur(data)" class="edit-input" @keyup.enter="inputPressEnter(data)"
                    @mousedown="handleMousedown" @click.stop v-focus ref="renameInputRef" />
                  <div v-else class="node-box__name">{{ name }}</div>
                </div>
              </div>
            </div>
            <div class="tree-view-right" v-if="isEditable" :id="`${posKey}`">
              <a-popover trigger="click" placement="bottomRight" :open="posKey === selectedData.posKey && actionsVisible"
                overlayClassName="tree-action-list-popover" :destroyTooltipOnHide="true">
                <template #content>
                  <ul class="action-list">
                    <template v-for="(option, index) in barOptions" :key="index">
                      <li v-if="option.isShow(data)" @click="option.function(data)" :class="`${option.type}`">
                        <a :class="`actions-item actions-item--${option.type}`">{{ option.label }} <span class="hot-key"
                            v-if="option.hotKey">{{ option.hotKey }}</span>
                        </a>
                      </li>
                    </template>
                  </ul>
                </template>
                <span class="btn-popup icon-list" @click.stop="openList(data)" v-click-outside="onClickOutside">
                </span>
              </a-popover>
            </div>
          </div>
      ...
      <p v-else-if="showTip && tempTreeData.length === 0" class="tip-text"> No results found </p>
    </div>
    <div v-else class="empty-block-wrapper">
      <div class="empty-block" v-if="!currentWorkspace.id">
        <div class="empty-block__header">No workspace</div>
      </div>
      <div class="empty-block" v-else>
        <div class="empty-block__header"> Create a collection for your requests </div>
        ...
        <div class="empty-block__button" v-if="isEditable">
          <a-button class="" @click="addCollection">
            <template #icon><plus-outlined /></template> Create Collection </a-button>
        </div>
      </div>
    </div>
  </div>
  <tip-dialog v-if="isShowTipDialog" :dialogVisible="isShowTipDialog" @close-tip-dialog="isShowTipDialog = false"
    :deleted-data="selectedData" :tree-data="tempTreeData" />
  <export-dialog v-if="isShowExportDialog" :dialogVisible="isShowExportDialog"
    @close-export-dialog="isShowExportDialog = false" :collection-Info="selectedData" />
</template>
<script>
import {
  watch,
...
export default {
  ...
  setup(props, context) {
    ...
    const rename = (data) => {
      selectedKey.value = selectedData.value.posKey;
    ...
    const openTab = (node) => {
      selectedData.value = node;
      // store.commit("SET_TREE_SELECTED_TAB", node); // 放 treeConfig
    ...
    const onDrop = async (info) => {
      ...
      if (!info.dropToGap) {
        ...
        if (dragNodeType === TypeEnum.item.name) {
          // item更換父集合
          if (dropNodeType === TypeEnum.item.name) return; // item不能放置item項下
          ...
          // folder更換父集合
          if (dropNodeType === TypeEnum.item.name) return; // folder不能放置item項下
    ...
    watch(
      ...
      (newVal, oldVal) => {
        ...
        selectedKey.value = currentTabData.value.posKey;
      },
      {
        immediate: true,
      }
    ...
    const duplicate = Helper.throttle(async (node) => {
      let res = {};
      const obj = {
        originalWorkspaceId: currentWorkspace.value.id,
        workspaceId: currentWorkspace.value.id,
    ...
    const getNewTree = (tree, pos) => {
      ...
      return tree.map((node, i) => {
        ...
        // 如果節點有子節點，遞迴調用 getNewTree 來位子節點添加 key 屬性
        ...
        if (newNode.item && newNode.item.length > 0) {
          ...
          newNode.item.forEach((e, index) => {
            if (e) {
              ...
              e.workspaceId = currentWorkspace.value.id;
...
.sidebar-tree {
  ...
  .ant-tree {
    height: calc(100% - #{$operation-height} - #{$operation-margin}) !important;
