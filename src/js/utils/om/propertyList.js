...
let __PARENT = '__parent'
let DEFAULT_INDEX_ATTR = 'id'
let DEFAULT_INDEXCASE_ATTR = false
let DEFAULT_INDEXMULTI_ATTR = false

// let PropertyList;

class PropertyList extends _PropertyBase {
  constructor(type, parent, populate) {
    super(type, parent, populate);
  ...
  _.getOwn(type, '_apicat_propertyIndexKey') && (this._apicat_listIndexKey = type._apicat_propertyIndexKey);

  // if the type has case sensitivity flags, set the same
  _.getOwn(type, '_apicat_propertyIndexCaseInsensitive') && (this._apicat_listIndexCaseInsensitive =
      type._apicat_propertyIndexCaseInsensitive);

  // if the type allows multiple values, set the flag
  _.getOwn(type, '_apicat_propertyAllowsMultipleValues') && (this._apicat_listAllowsMultipleValues =
      type._apicat_propertyAllowsMultipleValues);
...
Object.assign(PropertyList.prototype, {
  /**
   * Indicates that this element contains a number of other elements.
   *
   * @private
   */
  _apicat_propertyIsList: true,
   ...
   * @type {String}
   */
  _apicat_listIndexKey: DEFAULT_INDEX_ATTR,
   ...
   * @type {String}
   */
  _apicat_listIndexCaseInsensitive: DEFAULT_INDEXCASE_ATTR,
   ...
   * @type {String}
   */
  _apicat_listAllowsMultipleValues: DEFAULT_INDEXMULTI_ATTR,
  ...
  insert: function (item, before) {
    ...
    if ((index = item[this._apicat_listIndexKey]) && (index = String(index))) {
      // desensitise case, if the property needs it to be
      this._apicat_listIndexCaseInsensitive && (index = index.toLowerCase());

      // if multiple values are allowed, the reference may contain an array of items, mapped to an index.
      if (this._apicat_listAllowsMultipleValues && Object.hasOwnProperty.call(this.reference, index)) {
  ...
  upsert: function (item) {
    ...
    var indexer = this._apicat_listIndexKey,
  ...
  remove: function (predicate, context) {
    ...
    if (_.isString(predicate)) {
      ...
      match = this._apicat_listIndexCaseInsensitive ? predicate.toLowerCase() : predicate;
      predicate = function (item) {
          var id = item[this._apicat_listIndexKey];

          this._apicat_listIndexCaseInsensitive && (id = id.toLowerCase());
    ...
    _.isFunction(predicate) && _.remove(this.members, function (item) {
      ...
      if (predicate.apply(context, arguments)) {
        if ((index = item[this._apicat_listIndexKey]) && (index = String(index))) {
            ...
            if (this._apicat_listAllowsMultipleValues && _.isArray(this.reference[index])) {
  ...
  assimilate: function (source, prune) {
    var members = PropertyList.isPropertyList(source) ? source.members : source,
        list = this,
        indexer = list._apicat_listIndexKey,
   ...
   */
  one: function (id) {
    var val = this.reference[this._apicat_listIndexCaseInsensitive ? String(id).toLowerCase() : id];

    if (this._apicat_listAllowsMultipleValues && Array.isArray(val)) {
  ...
  has: function (item, value) {
    ...
    match = _.isString(item) ?
      this.reference[this._apicat_listIndexCaseInsensitive ? item.toLowerCase() : item] :
    ...
    if (this._apicat_listAllowsMultipleValues && _.isArray(match)) {
  ...
  /**
   * Converts a list of Properties into an object where key is `_apicat_propertyIndexKey` and value is determined
  ...
  toObject: function (excludeDisabled, caseSensitive, multiValue, sanitizeKeys) {
    ...
    key = this._apicat_listIndexKey,
    sanitiseKeys = this._apicat_sanitizeKeys || sanitizeKeys,
    sensitive = !this._apicat_listIndexCaseInsensitive || caseSensitive,
    multivalue = this._apicat_listAllowsMultipleValues || multiValue;
  ...
  toJSON: function () {
    ...
    return _.map(this.members, function (member) {
      ...
      return _.reduce(member, function (accumulator, value, key) {
          ...
          if (value && value._apicat_propertyIsList && !value._apicat_proprtyIsSerialisedAsPlural &&
              _.endsWith(key, 's')) {
              key = key.slice(0, -1);
          }
...
Object.assign(PropertyList, {
   ...
   * @type {String}
   */
  _apicat_propertyName: 'PropertyList',
  ...
  isPropertyList: function (obj) {
    return Boolean(obj) && ((obj instanceof PropertyList) ||
      _.inSuperChain(obj.constructor, '_apicat_propertyName', PropertyList._apicat_propertyName));
  }
